   ...
   <>   
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      <div className='absolute top-0 left-0 max-w-full z-1 bg-gradient-to-b from-cyan-100/70'></div>
        {/* left col */}
        <div className='max-w-x1 z-10 text-gray-500 '>
          <h1 className='md:text-lg text-course-details-small font-semibold text-gray-800 '>{courseData.courseTitle}</h1>
...
<div className='pt-8 text-gray-800 '>
  <h2 className='text-x1 font-semibold'>Course Structure</h2>
  <div className='pt-5'>
          ...
          <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-600'>
            {chapter.chapterContent.map((lecture , i)=>(
              <li key={i} className='flex items-start gap-2 py-1'>
                ...
                <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'><p>{lecture.lectureTitle}</p></div>
                <div className='flex gap-2'>{lecture.isPreviewFree  && <p  onClick={()=>setPlayerData({videoId: lecture.lectureUrl.split('/').pop()})} 
                className='text-blue-500 cursor-pointer' > Preview</p>}
                <p>{humanizeDuration(lecture.lectureDuration * 60 * 1000,{units:['h','m']})}</p></div>
              </li>
            ))}
          </ul>
          </div>
      ...
      <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
        ...
        <div className='pt-5'>
          ...
          <div className='flex gap-3 items-center pt-2'>
            ...
            <p className='md:text-lg text-gray-500 line-through'>{courseData.discount}% off</p>
          </div>
          <div className='flex items-center text-sm md: text-default gap-4 pt-2 md:pt-4 text-gray-500'>
            <div className='fkex items-center gap-1'>
          ...
          <div className='pt-6'>
            ...
            </p><ul className='ml-4 pt-2 text-sm md:text-default list-disc text-gray-500'>
              <li>Lifetime access with free updates.</li>
              <li>Step by step ,hands-on project guidence.</li>
              <li>Downloadable resources.</li>
              <li>
                Test your knowledge.
              </li><li>Certificate of complition.</li>
            </ul>
          </div>
        </div>
      </div>
